...
async def dyanamic_filtering(context:ToolFilterContext, tool):
    print("CONTEXT", context.run_context)
    print("TOOL", tool)
    # print("tool.name.startwith", tool.name.startwith("special"))
    # return tool.name.startwith("special")
    return True
...
async def main():
    ...
    async with MCPServerStreamableHttp(params=params_config, tool_filter=dyanamic_filtering, cache_tools_list= True) as mcp_hello_server:
